...
<html lang="en">
    ...
    <body class="nav-fixed">
                ...
                <nav class="drawer accordion drawer-light bg-white" id="drawerAccordion">
                    <div class="drawer-menu">
                        <div class="nav">
                            ...
                            <a class="nav-link active" href="mapel">
            ...
            <div id="layoutDrawer_content">
                ...
                <main>
                    <div class="container-xl px-5">
                        ...
                        <h2 class="mb-4 mt-5">Tambah Data Mata Pelajaran</h2>
                        
                        <form>
                            <div class="mb-4"><mwc-textfield class="w-100" id="nama_mapel" label="Nama Mata Pelajaran" outlined=""></mwc-textfield></div>
                        ...
                        <h2 class="mb-4 mt-5">Ubah Data Mata Pelajaran</h2>
                        
                        <form>
                            <input type="hidden" id="id">
                            <div class="mb-4"><mwc-textfield class="w-100" id="nama_mapel_update" label="Nama Mata Pelajaran" outlined=""></mwc-textfield></div>
                            <div class="form-group d-flex align-items-center justify-content-between mt-4 mb-7">
                                <a class="small fw-500 text-decoration-none"></a>
                        ...
                        <h2 class="mb-4 mt-5">Data Mata Pelajaran</h2>
                        ...
                        <table id="datatablesSimple">
                            ...
                            <tbody>
                                @php
                                    $position = 1;
                                @endphp
                                @foreach ($response->mapel as $output)
                                    <tr>
                                        <td>{{ $position++ }}</td>
                                        <td>{{ $output->nama_mapel }}</td>
                                        <td>
                                            <button type="button" class="btn btn-danger" onclick="hapus({{ $output->id }})">HAPUS</button>
                                            <button type="button" class="btn btn-primary" onclick="berubah('{{ $output->id }}', '{{ $output->nama_mapel }}')">UBAH</button>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
        ...
        <script>
            ...
            function buat() {
                const mapel = document.getElementById('nama_mapel').value;
                ...
                if (mapel === '') {
                    ...
                    fetch('{{ $url }}/api/mapel', {method: 'POST',body: JSON.stringify(data),headers: {'Content-Type': 'application/json'}})
            ...
            function ubah() {
                const mapel = document.getElementById('nama_mapel_update').value;
                const id = document.getElementById('id').value;

                if (id === '') {
                    alert('Silahkan pilih "UBAH" disalah satu data');
                } else if (mapel === '') {
                    alert('Mata Pelajaran Tidak Boleh Kosong');
                } else {
                    const data = {
                    ...
                    fetch('{{ $url }}/api/mapel_update', {method: 'POST',body: JSON.stringify(data),headers: {'Content-Type': 'application/json'}})
                    .then(response => response.json())
                    .then(data => {
                        alert(data.catatan);
                        location.reload(data.aksi);
            ...
            function berubah(id, nama) {
                document.getElementById('nama_mapel_update').value = nama;
                document.getElementById('id').value = id;
            }

            function hapus(id) {
                fetch('{{ $url }}/api/mapel/' + id, {method: 'DELETE',headers: {'Content-Type': 'application/json'}})
                    .then(response => response.json())
                    .then(data => {
                        alert(data.catatan);
                        location.reload(data.aksi);
