    ...
    <body class="nav-fixed">
                ...
                <nav class="drawer accordion drawer-light bg-white" id="drawerAccordion">
                    <div class="drawer-menu">
                        <div class="nav">
                            ...
                            <a class="nav-link active" href="guru">
            ...
            <div id="layoutDrawer_content">
                ...
                <main>
                    <div class="container-xl px-5">
                        ...
                        <h2 class="mb-4 mt-5">Tambah Data Guru</h2>
                        
                        <form>
                            <div class="row">
                                <div class="col-sm-6 mb-4"><mwc-textfield class="w-100" id="nama_lengkap" label="Nama Lengkap" outlined=""></mwc-textfield></div>
                                <div class="col-sm-6 mb-4">
                                    <select class="form-select form-select-lg" id="pilih_mapel" aria-label="Default select options">
                                        <option value="">Pilih Mata Pelajaran</option>
                                        @foreach ($response_mapel->mapel as $output)
                                            <option value="{{ $output->id }}">{{ $output->nama_mapel }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-sm-6 mb-4"><mwc-textfield class="w-100" id="username" label="Username" outlined=""></mwc-textfield></div>
                                <div class="col-sm-6 mb-4"><mwc-textfield class="w-100" id="password" label="Password" outlined="" icontrailing="visibility_off" type="password"></mwc-textfield></div>
                            </div>
                            <div class="form-group d-flex align-items-center justify-content-between mt-4 mb-7">
                                <a class="small fw-500 text-decoration-none"></a>
                                <button class="btn btn-primary" type="button" onclick="buat()">SIMPAN</button>
                            </div>
                        </form>

                        <h2 class="mb-4 mt-5">Ubah Data Guru</h2>
                        
                        <form>
                            <input type="hidden" id="id">
                            <div class="row">
                                <div class="col-sm-6 mb-4"><mwc-textfield class="w-100" id="nama_lengkap_update" label="Nama Lengkap" outlined=""></mwc-textfield></div>
                                <div class="col-sm-6 mb-4">
                                    <select class="form-select form-select-lg" id="pilih_mapel_update" aria-label="Default select options">
                                        <option value="">Pilih Mata Pelajaran</option>
                                        @foreach ($response_mapel->mapel as $output)
                                            <option value="{{ $output->id }}">{{ $output->nama_mapel }}</option>
                                        @endforeach
                            ...
                            <div class="row">
                                <div class="col-sm-6 mb-4"><mwc-textfield class="w-100" id="username_update" label="Username" outlined=""></mwc-textfield></div>
                                <div class="col-sm-6 mb-4"><mwc-textfield class="w-100" id="password_update" label="Password" outlined="" icontrailing="visibility_off" type="password"></mwc-textfield></div>
                            </div>
                            <div class="form-group d-flex align-items-center justify-content-between mt-4 mb-7">
                                <a class="small fw-500 text-decoration-none"></a>
                                <button class="btn btn-info" type="button" onclick="ubah()">SIMPAN</button>
                            </div>
                        </form>

                        <h2 class="mb-4 mt-5">Data User Guru</h2>
                        ...
                        <table id="datatablesSimple">
                            ...
                            <tbody>
                                ...
                                @foreach ($response->guru as $output)
                                    @foreach ($response_mapel->mapel as $o)
                                        @if ($o->id == $output->id_mapel)
                                            <tr>
                                                <td>{{ $position++ }}</td>
                                                <td>{{ $output->nama_guru }}</td>
                                                <td>{{ $o->nama_mapel }}</td>
                                                <td>{{ $output->username_guru }}</td>
                                                <td>
                                                    <button type="button" class="btn btn-danger" onclick="hapus({{ $output->id }})">HAPUS</button>
                                                    <button type="button" class="btn btn-primary" onclick="berubah('{{ $output->id }}', '{{ $output->nama_guru }}', '{{ $output->id_mapel }}', '{{ $output->username_guru }}', '{{ $output->password_guru }}')">UBAH</button>
                                                </td>
                                            </tr>
                                        @endif
                                    @endforeach
        ...
        <script>
            ...
            function buat() {
                ...
                const pilih_mapel = document.getElementById('pilih_mapel').value;
                ...
                if (nama_lengkap === '') {
                    ...
                    alert('Pilih Mata Pelajaran Tidak Boleh Kosong');
                } else if (username === '') {
                    ...
                    fetch('{{ $url }}/api/guru', {method: 'POST',body: JSON.stringify(data),headers: {'Content-Type': 'application/json'}})
            ...
            function ubah() {
                const nama_lengkap = document.getElementById('nama_lengkap_update').value;
                const pilih_mapel = document.getElementById('pilih_mapel_update').value;
                ...
                if (id === '') {
                    alert('Silahkah Pilih "Ubah" Pada Data Yang Ada');
                    ...
                    const data = {
                        id: id,
                        nama: nama_lengkap,
                        id_mapel: pilih_mapel,
                        user: username,
                        pass: password
                    };

                    fetch('{{ $url }}/api/guru_update', {method: 'POST',body: JSON.stringify(data),headers: {'Content-Type': 'application/json'}})
                    .then(response => response.json())
                    .then(data => {
                        alert(data.catatan);
                        location.reload(data.aksi);
            ...
            function berubah(id, nama, pilih, username, password) {
                document.getElementById('id').value = id;
                document.getElementById('nama_lengkap_update').value = nama;
                document.getElementById('pilih_mapel_update').value = pilih;
                document.getElementById('username_update').value = username;
                document.getElementById('password_update').value = password;
            }

            function hapus(id) {
                fetch('{{ $url }}/api/guru/' + id, {method: 'DELETE',headers: {'Content-Type': 'application/json'}})
                    .then(response => response.json())
                    .then(data => {
                        alert(data.catatan);
                        location.reload(data.aksi);
